...
app.post('/', (req, res) => {
  try {
    ...
    const oPayload = {
      appKey: process.env.ZOOM_MEETING_SDK,
      sdkKey: process.env.ZOOM_MEETING_SDK,
      mn: meetingNumber,
      role,
      iat,
      exp,
    ...
    const sdkJWT = KJUR.jws.JWS.sign('HS256', sHeader, sPayload, process.env.ZOOM_MEETING_SECRET)
    return res.json({ signature: sdkJWT, sdkKey: process.env.ZOOM_MEETING_SDK })
  } catch (error) {
    console.log(error)
  }
})
